...
class FacebookGroupsHarvestTask extends Shell
{
    ...
    public function main()
    {
    	...
    	$this->out("FacebookGroupsHarvest:ok");
    ...
    public function harvest(){
		...
		$stmt = $conn->execute('SELECT * FROM research_elements re WHERE re.research_element_type_id=7 AND ACTIVE=1 ORDER BY updated_last ASC LIMIT 0,1');
		...
		if(count($rows)>0){
			...
			if(count($rows3)>0){
				...
				if(isset($fbAppId) && isset($fbAppSecret) && $fbAppId!="" && $fbAppSecret!=""){
					foreach ($ids as $id) {
						//echo("ID-->" . $id . "\n\n\n");
						//echo('python FB/get_fb_posts_fb_group.py ' . $id . ' ' . $fbAppId . ' ' . $fbAppSecret);
						//echo("\n\n\n");

						exec('python FB/get_fb_posts_fb_group.py ' . $id . ' ' . $fbAppId . ' ' . $fbAppSecret, $output);
						...
						if ($handle) {
						    while (!feof($handle)) {
						        ...
						        if(!$isfirst && count($parts)==16){

						        	

						        	$id_subject = -1;
						        	$id_str = str_replace("'", " ", $parts[2]);
									$name = str_replace("'", " ", $parts[2]);
									$screen_name = str_replace("'", " ", $parts[2] );
									$location = "";
									$followers_count = $parts[7] + $parts[8] + $parts[9] + $parts[10];
									$friends_count = 0;
									$listed_count = 0;
									$lang =  "XXX";
